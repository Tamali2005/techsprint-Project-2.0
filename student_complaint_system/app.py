...
# 🔴 IMPORTANT: Paste Supabase connection string here
DATABASE_URL = "postgresql://postgres:[Tamajit@2004]@db.nbfpfpensjhliurynqyt.supabase.co:5432/postgres"
...
def submit_complaint():
    try:
        ...
        """, (
            session['student_id'], student['full_name'], student['email'], student['phone'],
            student['department'], student['identity_type'],
            request.form.get('categoryType'),
            request.form.get('complaintType'),
            request.form.get('complaintTitle'),
            request.form.get('detailedDescription'),
            file_path,
            request.form.get('complaintMode', 'Online'),
            request.form.get('priority', 'Medium')
        ))
